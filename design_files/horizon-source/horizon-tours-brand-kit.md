...
| Primary | Horizon Blue `#4F5BFF` | Horizon Sunset `#FF8A3D` |
| Accent | Aurora `#FF8A3D` (rare, = Tours primary) | Deep Blue `#4F5BFF` (rare, sibling-link, = Connect primary) |
...
- **Status ramp anchors** — success/warning/danger/info ramps unchanged. (Tours uses `--feedback-warning-*` for warnings just like Connect. The Aurora accent and Tours' primary are the same hue — `#FF8A3D` — which is the visual grammar of the sibling-brand link.) been promoted to primary, so warnings need their own dedicated ramp as before.)
...
Tours' primary is **`#E0691C`** at `orange-600` (the product anchor that clears WCAG AA on white) with **`#FF8A3D`** at `orange-500` (the marketing-vivid stop). This mirrors Connect's pattern: a deeper "product" version that holds up under text-on-button contrast, and a brighter "marketing" version that owns hero panels and ad creative.

The `#FF8A3D` you'll see on horizontours.com hero sections is the marketing-vivid. The colour inside the booking app at `Book this experience` CTAs is `#E0691C`. Same family, calibrated for context.

**Why sunset orange instead of, say, a Rockies-water teal:** orange/gold is the colour of golden-hour light hitting Cascade Mountain, the Canadian Rockies' most reproduced tourism photograph. It's emotionally pre-loaded for the destination in a way that engineered teal or forest green isn't.WCAG AA contrast on white, and a richer "marketing" version used on hero panels and ad creative where the contrast requirement is looser.

The `#FF8A3D` you'll see on horizontours.com hero sections is the marketing-vivid. The amber colour inside the booking app at `Book this experience` CTAs is `#E0691C`. Same family, calibrated for context.
...
Connect's neutrals carry a subtle cool undertone (~2% blue in the grey). Tours' neutrals carry a subtle warm undertone (~3% orange). This is what makes the entire UI feel tonally connected to the primary colour.rimary instead of looking like brand colour layered on generic grey.
...
Connect's shadows are `rgba(14, 14, 20, ...)` — a tiny bit of cool blue mixed in. Tours' shadows are `rgba(20, 18, 14, ...)` — a tiny bit of warm brown mixed in. Imperceptible in isolation; felt cumulatively. as tonal coherence across the UI.

### 6. Focus ring is orange, not blue

Keyboard focus on Tours surfaces uses an orange ring (`rgba(224, 105, 28, 0.18)`) instead of blue. Same job, branded for the right product.

---

## The sibling-link accent

Deep Blue (`#4F5BFF`) — Connect's primary — appears in the Tours system as the **rare accent**, with ≤1% pixel coverage. Reserved for:
...
Use the `.sibling-accent` class. Don't reach for `--color-brand-deep-blue` anywhere else.

The reverse mirrors: Aurora orange appears in Connect's system at ≤2% pixel coverage as the rare celebration accent — same hex (`#FF8A3D`) as Tours' primary. Each brand's primary is the other brand's rare accent. That's the visual grammar of sibling brands.f "siblings."
...
If you're building a unified component library that serves both products, components should reference semantic tokens only (`--action-primary-default`) and never primitives (`--color-blue-600` or `--color-orange-600`). The brand the component renders in is determined by which token file is loaded — the component itself stays brand-agnostic. This is exactly how Material 3 themes, Radix theming, and Shopify Polaris multi-brand work.

---

## Component build sequence

You don't need to rebuild any components for Tours. Components built for Connect work in Tours unchanged — same buttons, same cards, same forms — they just render in sunset orange with softer corners and warmer shadows.more padding because of the tokens flowing through them.
...
- **Don't add a third brand colour to Tours.** "What if we used a green for nature tours?" No. The brand is sunset orange. Different tour categories don't get different colours. Categorize via photography, copy, and layout — not by adding hues.py, and content — not chrome.
