...
- **One canonical violet.** `--color-violet-500` (`#5B2DE8` — "Horizon Violet") is the primary brand colour everywhere: product, marketing, decks. `--color-violet-600` (`#4920C4`) plays the *hover* role, `--color-violet-700` (`#3A189E`) the *pressed* role. The earlier `marketing-vivid` distinction has been collapsed — the brighter violet reads correctly in both registers and is friendlier for a hotel-facing surface.
